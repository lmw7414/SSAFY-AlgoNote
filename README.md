...
1. 문제를 풀고나서, 잊어버리지 않기 위해 정리를 해야하는데 항상 지쳐 정리는 뒷전이었던 적이 있으신가요?
2. 나에게 영양가 높은 딱 맞는 문제를 찾는데, 많은 시간을 허비한 적은 없으신가요?
3. 마지막으로 내가 놓치고 있는 알고리즘 유형에 대해서는 시각적으로 보이지 않아 불편하지 않으셨나요?

이러한 불편점을 느꼈으면 저희의 서비스를 이용해보세요. 
알고리즘 강화를 위한 최적의 노트! 알고노트 

### 1. 프로젝트를 어떻게 기획하게 되었나요?
알고리즘의 문제를 풀면서 풀었던 문제에 다시 한번 생각해보는 과정이 꼭 필요하다고 느낀 적이 있어요. 하지만 이러한 습관을 들이는 과정 자체가 너무 힘들었어요.
나의 코드를 다시 살펴보고, 실행시키기 위해 여러가지 기능들이 필요했어요. 또한 노트를 작성하면서도 여러 페이지와 툴들을 이동해가면서 정리를 해야했죠. 이 모든 것을 한 서비스 내에서 할 수 없을까 생각을 하게되었고, 서비스로 만들게 되었습니다.   

### 2. 프로젝트의 핵심 가치는 무엇인가요?
알고리즘을 풀고 난 후 노트 작성에 필요한 다양한 기능들을 제공해요. 익스텐션을 통해 백준에서 푼 코드가 저희 서비스로 바로 넘어오고, 저희 서비스는 마크다운을 지원하기 때문에 편리하게 노트 작성을 할 수 있어요. 또한 챗봇이 있어 노트 작성 중 궁금한 내용을 언제든 해소할 수 있습니다.
이 뿐일까요? 사용자의 알고리즘 풀이에 따른 진척도를 그래프를 통해 한 눈에 파악할 수 있어요. 그리고 그래프를 클릭하면 사용자에게 딱 맞는 영양가 높은 문제를 추천해줍니다. 문제 추천부터, 코드 분석, 노트 작성까지! 알고리즘 학습에 확실한 도움을 제공합니다.
